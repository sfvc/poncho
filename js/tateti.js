...
function restartGame(ask) {
    if (moves > 0) {
        var response = confirm("¿Estás seguro de reiniciar?");
        if (response === false) {
            return;
        }
    }
...
function endGame(who) {
    if (who == player) {
        announceWinner("¡Felicidades, Ganaste!");
    } else if (who == computer) {
        announceWinner("¡La computadora gana!");
    } else {
        announceWinner("¡Es un empate!");
    }
    gameOver = true;
    whoseTurn = 0;
    moves = 0;
